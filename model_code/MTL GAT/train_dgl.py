...
def run_a_train_epoch(args,epoch,model,data_loader,loss_criterion,optimizer):
    ...
    print('epoch {:d}/{:d}, training {} {:.4f}'.format(
        epoch + 1, args['num_epochs'], args['metric'], train_score))  # 打印当前 epoch 的训练指标得分
    if args['n_tasks'] > 1:
...
def run_a_val_epoch(args,model,data_loader,loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            masks = masks.to(device)
            n_feats = bg.ndata.pop('hv').to(device)
            val_prediction = model(bg,n_feats)
            val_loss = (loss_criterion(val_prediction,labels) * (masks != 0).float()).mean()
...
def train(args):
    ...
    ###将图数据划分成多个batch
    train_dataloader = DataLoader(dataset=train_set,batch_size=args['batch_size'],shuffle=True,collate_fn=collate_molgraphs)
    valid_dataloader = DataLoader(dataset=valid_set,batch_size=args['batch_size'],collate_fn=collate_molgraphs)
    ...
    torch.save(model.state_dict(),os.path.join(args['result_path'],'MultiGAT_{}_model.pt'.format(args['seed'])))
...
if __name__ == '__main__':
    ...
    if args['task_names'] is not None:
        args['task_names'] = args['task_name'].split(',') ###使用逗号分隔开
    ...
    for num_fold in range(args['num_folds']):
        ...
        best_val,best_train,test_auc, test_acc, test_recall, test_precision, test_f1, test_bacc, test_sp, test_mcc = train(args)
...
total_auc.to_csv('Multi_GAT_model/10_seeds_AUC_GAT.csv',index=False)
acc_df = pd.DataFrame(test_acc_list,columns=args['task_names'])
acc_df.to_csv('Multi_GAT_model/10_seeds_ACC_GAT.csv',index=False)
recall_df = pd.DataFrame(test_recall_list,columns=args['task_names'])
recall_df.to_csv('Multi_GAT_model/10_seeds_Recall_GAT.csv',index=False)
pre_df = pd.DataFrame(test_precision_list,columns=args['task_names'])
pre_df.to_csv('Multi_GAT_model/10_seeds_precision_GAT.csv',index=False)
f1_df = pd.DataFrame(test_f1_list,columns=args['task_names'])
f1_df.to_csv('Multi_GAT_model/10_seeds_f1_score_GAT.csv',index=False)
bacc_df = pd.DataFrame(test_bacc_list,columns=args['task_names'])
bacc_df.to_csv('Multi_GAT_model/10_seeds_BACC_GAT.csv',index=False)
sp_df = pd.DataFrame(test_sp_list,columns=args['task_names'])
sp_df.to_csv('Multi_GAT_model/10_seeds_Specificity_GAT.csv',index=False)
mcc_df = pd.DataFrame(test_mcc_list,columns=args['task_names'])
mcc_df.to_csv('Multi_GAT_model/10_seeds_MCC_GAT.csv',index=False)
