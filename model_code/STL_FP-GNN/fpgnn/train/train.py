...
def fold_train(args, log):
    ...
    ###单任务模型
    args.train_path = os.path.join(args.data_path,f'{args.seed}_3A4_train.csv')
    args.val_path = os.path.join(args.data_path,f'{args.seed}_3A4_valid.csv')
    args.test_path = os.path.join(args.data_path,f'{args.seed}_3A4_test.csv')
    args.task_names = get_task_name(args.train_path) #获取任务名称,1A2
    data = load_data(args.train_path,args)  #包含训练集的MoleDataset对象，里面包含所有有效的MoleData
    args.task_num = data.task_num() #获取数据中的任务数量,1
    ...
    train_data = load_data(args.train_path, args)
    ...
    debug(f'Train size: {len(train_data)}    Val size: {len(val_data)}   Test size: {len(test_data)}')  ###改
    ...
    ###steps_per_epoch步数是指在每个epoch中，模型更新参数的次数
