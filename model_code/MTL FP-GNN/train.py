...
def training(args,log):
    ...
    valid_df = pd.DataFrame(valid_scores,columns=['valid_1A2','valid_2C9','valid_2C19','valid_2D6','valid_3A4'])
    test_df = pd.DataFrame(test_scores,columns=['test_1A2','test_2C9','test_2C19','test_2D6','test_3A4'])
    total_df = pd.concat([train_df,valid_df,test_df],axis=1)
    total_df.to_csv('FP_GNN_10_seeds_result.csv')
    core = np.array(test_scores)
