...
def add_train_argument(p):
    ...
    p.add_argument('--is_multitask',type=int,default=1,
                   help='Whether the dataset is multi-task. 0:no  1:yes.')
    #使用这个参数选项的目的是让用户在命令行中指定多任务训练中的任务数量
    #添加一个多任务数量的参数，默认值为1
    p.add_argument('--task_num',type=int,default=5,
...
def add_predict_argument(p):
    p.add_argument('--predict_path', type=str,default='Seed_0_domain',
    ...
    p.add_argument('--model_path', type=str,default='FPGNN_model_save_random_最终结果',
...
def add_intergraph_argument(p):
    p.add_argument('--predict_path', type=str,default='final_external.csv',
                   help='The path of input CSV file to predict.')
    p.add_argument('--figure_path', type=str,default='figure_seed_0',
